...
public class Column
{
    ...
    public String getSqlString()
    {
        ...
        resultList.add(this.getPlatform().handleColumnName(getName()));

//        String type = getDomain().getSqlType();
        String type = this.getPlatform().convertType(domain);
        ...
        if (getPlatform().hasSize(getDomain().getSqlType()))
        {
            ...
            if(fk != null)
            {
                String foreignname = (String)fk.getLocalForeignMapping().get(this.getName());
                String ftbl = fk.getForeignTableName();
                Table tbl = this.getTable().getDatabase().getTable(ftbl);
                if(tbl != null)
                	type += tbl.getColumn(foreignname).getDomain().printSize();
                else
                {
                	ColumnMetaData cl = DBUtil.getColumnMetaData(this.parentTable.getTableParent().getName(), ftbl, foreignname);
                	if(cl.getDECIMAL_DIGITS() == 0)
                	{
                		type += Domain.printSize(cl.getColunmSize()+"",null);
                	}
                	else
                	{
                		type += Domain.printSize(cl.getColunmSize()+"",cl.getDECIMAL_DIGITS()+"");
                	}
...
//            }
//        }
        String  defaultValue = this.getPlatform().getDefaultValue(this.getDomain());
